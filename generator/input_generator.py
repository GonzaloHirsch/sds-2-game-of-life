...
def generate_static_file(filename, area_length, layers):
    f = open(filename, 'w')
...
#   - area_length -> Total length of the area of study
def generate_dynamic_file(filename, area_length):
    # Obtains the usable points placed in the middle of the area of study
    points = obtain_points_from_matrix(area_length)
...
def obtain_points_from_matrix(area_length):
    ...
    index, dim, dims, l_count, l_expected = 0, 0, 2, 0, 0
    for line in ic:
        if index == 0:
...
generate_files(area_length, layers)